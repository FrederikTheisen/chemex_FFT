...
class Buttons:
    ...
    LSTYLE = {"a": "-", "b": ":", "c": "dashdot"}
    TEXT_Y = {"a": 0.8, "b": 0.75, "c": 0.7}
    ...
    def _add_text_dw(self, dw, state):
        text_ = rf"$\Delta\varpi_{{a{state}}}$ = {dw:.3f} ppm"
        text = self.fig.text(0.82, self.TEXT_Y[state]-.1, text_)
    ...
    def _plot_lines(self, event=None):
        ...
        if xdata is not None:
            if cs_a is None:
                cs_a = xdata
            elif cs_b is None or not self.use_c:
                cs_b = xdata
            elif cs_c is None:
                cs_c = xdata
            else:
        ...
        if cs_b is not None:
            ...
            self._add_text_dw(dw_ab, "b")
            
        if cs_c is not None:
            dw_ac = cs_c - cs_a
            self._add_line(cs_c, "c")
            self._add_text_dw(dw_ac, "c")

        self.cs_a[self.name] = cs_a
        self.cs_b[self.name] = cs_b
        if self.use_c: self.cs_c[self.name] = cs_c
    ...
    def __init__(self, experiments, path, sw, use_c = False):
    ...
    def clear(self, event):
        name = self.name
        self.cs_a[name], self.cs_b[name], self.cs_c[name] = None, None, None
        self._plot_lines()
...
def pick_cest(args: Namespace):
    ...
    callback = Buttons(experiments, args.out_dir, sw, args.model == "3st")
