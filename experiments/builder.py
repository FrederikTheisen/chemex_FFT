...
def build_experiment(filename: Path, selection: Selection, useac: bool = False) -> Experiment:
    config_dict: MutableMapping[str, Any] = read_toml(filename)

    experiment_name = _get_experiment_name(config_dict, filename)
    if useac: #FFT AC model code
        experiment_name += "_ac"
        config_dict['experiment']['name'] += "_ac"
...
def build_experiments(filenames: list[Path] | None, selection: Selection, useac: bool = False):
    ...
    for filename in filenames:
        experiment = build_experiment(filename, selection, useac)
