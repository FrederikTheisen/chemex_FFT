...
def run2(args: Namespace):
    """Run the fit or simulation."""
    print("RUNNING FFT MODEL")
    # Parse kinetics model
    args.model = args.model[0:-3] #fix model name
    ...
    # Read experimental setup and data
    selection = Selection(args.include, args.exclude)
    experiments = build_experiments(args.experiments, selection, True)
...
def main():
    ...
    if args.commands is None:
        parser.print_help()
    elif "model" in args and 'ac' in args.model:
        run2(args)
    else:
        args.func(args)
