...
class Experiments:
    ...
    def calculate(self, params: Parameters) -> np.ndarray: #FFT
        return np.asarray(
            list(
                chain.from_iterable(experiment.values(params) for experiment in self)
            )
        )

    def getdata(self): #FFT
        return list(chain.from_iterable(experiment.data() for experiment in self))
