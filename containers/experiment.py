...
class Experiment:
    ...
    def values(self, params : ParametersLF) -> list[float]: #FFT
        return list(
            chain.from_iterable(profile.maskedcalculate(params) for profile in self.profiles)
        )

    def data(self) -> list[float]: #FFT
        return chain.from_iterable(profile.getdata() for profile in self.profiles)
